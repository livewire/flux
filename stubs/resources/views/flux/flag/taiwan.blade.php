@blaze(fold: true)

{{-- Credit: Nucleo App (https://nucleoapp.com/svg-flag-icons) --}}

@props([
    'size' => 'base',
])

@php
    $classes = Flux::classes('shrink-0')->add(
        match ($size) {
            'xl' => '[:where(&)]:size-16',
            'lg' => '[:where(&)]:size-12',
            'base' => '[:where(&)]:size-8',
        },
    );
@endphp

<svg {{ $attributes->class($classes) }}
     data-flux-flag
     data-slot="flag"
     viewBox="0 0 32 32"
     xmlns="http://www.w3.org/2000/svg">
    <rect fill="#e93323"
          height="24"
          rx="4"
          ry="4"
          width="30"
          x="1"
          y="4"></rect>
    <path d="M5,4H16v12H1V8c0-2.208,1.792-4,4-4Z" fill="#020090"></path>
    <path d="M27,4H5c-2.209,0-4,1.791-4,4V24c0,2.209,1.791,4,4,4H27c2.209,0,4-1.791,4-4V8c0-2.209-1.791-4-4-4Zm3,20c0,1.654-1.346,3-3,3H5c-1.654,0-3-1.346-3-3V8c0-1.654,1.346-3,3-3H27c1.654,0,3,1.346,3,3V24Z"
          opacity=".15"></path>
    <path d="M27,5H5c-1.657,0-3,1.343-3,3v1c0-1.657,1.343-3,3-3H27c1.657,0,3,1.343,3,3v-1c0-1.657-1.343-3-3-3Z"
          fill="#fff"
          opacity=".2"></path>
    <path d="M6.664,8.859l-1.953-.523,1.429,1.429c.109-.339,.29-.647,.524-.906Z" fill="#fff"></path>
    <path d="M8.5,8.044c.179,0,.354,.02,.523,.056l-.523-1.952-.523,1.952c.169-.036,.344-.056,.523-.056Z" fill="#fff">
    </path>
    <path d="M7.741,8.163l-1.429-1.429,.523,1.953c.258-.234,.566-.415,.906-.524Z" fill="#fff"></path>
    <path d="M10.164,8.687l.523-1.953-1.429,1.429c.339,.109,.647,.29,.906,.524Z" fill="#fff"></path>
    <path d="M10.86,9.764l1.429-1.429-1.953,.523c.234,.258,.415,.566,.524,.906Z" fill="#fff"></path>
    <circle cx="8.5"
            cy="10.523"
            fill="#fff"
            r="2.188"></circle>
    <path d="M12.875,10.523l-1.952-.523c.036,.169,.056,.344,.056,.523s-.02,.354-.056,.523l1.952-.523Z" fill="#fff">
    </path>
    <path d="M9.259,12.883l1.429,1.429-.523-1.953c-.258,.234-.566,.415-.906,.524Z" fill="#fff"></path>
    <path d="M10.336,12.187l1.953,.523-1.429-1.429c-.109,.339-.29,.647-.524,.906Z" fill="#fff"></path>
    <path d="M8.5,13.002c-.179,0-.354-.02-.523-.056l.523,1.952,.523-1.952c-.169,.036-.344,.056-.523,.056Z"
          fill="#fff"></path>
    <path d="M6.836,12.359l-.523,1.953,1.429-1.429c-.339-.109-.647-.29-.906-.524Z" fill="#fff"></path>
    <path d="M6.14,11.282l-1.429,1.429,1.953-.523c-.234-.258-.415-.566-.524-.906Z" fill="#fff"></path>
    <path d="M6.021,10.523c0-.179,.02-.354,.056-.523l-1.952,.523,1.952,.523c-.036-.169-.056-.344-.056-.523Z"
          fill="#fff"></path>
</svg>
